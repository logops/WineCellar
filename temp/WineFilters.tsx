...
export function WineFilters({ 
  ...
  // Add a click outside handler that closes the filter panel
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (isOpen) {
        // Check if the click was outside the filter panel
        const filterPanel = document.getElementById('wine-filter-panel');
  ...
  if (!isOpen) {
    return (
      <Button 
        variant="outline" 
        size="sm" 
        className="fixed left-0 top-1/2 -translate-y-1/2 bg-white shadow-md border-r-0 rounded-l-md rounded-r-none pl-2 pr-1 py-6 z-10"
        onClick={onToggle}
      >
        <Filter size={18} />
      </Button>
