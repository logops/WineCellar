...
// Simple wine card component that doesn't depend on other imports
function WineCard({ wine, isSelected, onToggleSelect }: { 
  wine: any; 
  isSelected: boolean; 
  onToggleSelect: () => void;
}) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap mt-2 gap-x-4 gap-y-1 text-xs text-gray-500">
            <div>
              {wine.quantity} bottle{wine.quantity !== 1 ? 's' : ''} · {wine.bottleSize || '750ml'}
            </div>
            
            {wine.currentValue && (
              <div>
                Value: ${wine.currentValue.toFixed(2)}
              </div>
            )}
...
export default function SimpleRemoveByLabel() {
  ...
  const analyzeLabelMutation = useMutation({
    mutationFn: async () => {
      ...
      // We need to handle FormData specially with fetch
    ...
    onSuccess: async (response) => {
      try {
        ...
        setSelectedWineIds(new Set(matchedWines.map((wine: any) => wine.id)));
        ...
        toast({
          ...
          description: "There was a problem processing the server response.",
          variant: "destructive",
        });
      } finally {
